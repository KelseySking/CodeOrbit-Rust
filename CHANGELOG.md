...
### Fixed
- Claude Code 最新版双 hook（同一工具调用同时阻塞 PreToolUse 与 PermissionRequest）合并为一次待处理动作，允许 / 拒绝 / 超时按等待方分别回传。
- 优化 Claude 超时处理与问答响应构建。
...
### Added
- WSL hook installation support for bundled CLI sources.

### Fixed
- Improved VSCode terminal environment detection and process tracking.
